...
def query_des_catalogue(coord):
    ...
    try:
        v = VizieR(catalog='II/357/des_dr1', columns=['*', '+_e'])
        v.ROW_LIMIT = 1
        result = v.query_region(coord, radius=2 * u.arcsec)

        des_photometry = {
            'flux_DES_g': -999.0, 'flux_err_DES_g': -999.0,
            'flux_DES_r': -999.0, 'flux_err_DES_r': -999.0,
            'flux_DES_i': -999.0, 'flux_err_DES_i': -999.0,
            'flux_DES_z': -999.0, 'flux_err_DES_z': -999.0,
            'flux_DES_Y': -999.0, 'flux_err_DES_Y': -999.0
        }

        if not result or len(result[0]) == 0:
            print("  No DES DR1 source found within 2 arcsec.")
            return des_photometry

        source = result[0][0]
        bands = {'g': ('gmag', 'e_gmag'), 'r': ('rmag', 'e_rmag'), 'i': ('imag', 'e_imag'), 'z': ('zmag', 'e_zmag'), 'Y': ('Ymag', 'e_Ymag')}
        for band_key, (mag_col, mag_err_col) in bands.items():
            flux_band_name = f'flux_DES_{band_key}'
            flux_err_band_name = f'flux_err_DES_{band_key}'
            if mag_col in source.columns and source[mag_col] is not np.ma.masked:
                ...
                des_photometry[flux_band_name] = flux_ujy

                if mag_err_col in source.columns and source[mag_err_col] is not np.ma.masked:
                    mag_err = source[mag_err_col]
                    flux_err_ujy = flux_ujy * np.log(10) * 0.4 * mag_err
                    des_photometry[flux_err_band_name] = flux_err_ujy
                    print(f"  Found DES {band_key}-band flux: {flux_ujy:.2f} +/- {flux_err_ujy:.2f} uJy")
                else:
                    print(f"  Found DES {band_key}-band flux: {flux_ujy:.2f} uJy (no error)")
...
def perform_photometry(image_path, coord, instrument):
    """Performs aperture photometry on a single FITS image and calculates errors."""
    ...
    try:
        with fits.open(image_path, memmap=False) as hdul:
            ...
            sky_annulus = CircularAnnulus(position,
            ...
            if annulus_mask is None:
                print("  Warning: Annulus mask could not be created (possibly off-image). Setting background to 0.")
                bkg_median, bkg_std = 0.0, 0.0
            else:
                annulus_data = annulus_mask.get_values(data)
                finite_annulus_data = annulus_data[np.isfinite(annulus_data)]
                if len(finite_annulus_data) == 0:
                    print("  Warning: Sky annulus contains no valid data. Setting background to 0.")
                    bkg_median, bkg_std = 0.0, 0.0
                else:
                    bkg_median = np.median(finite_annulus_data)
                    bkg_std = np.std(finite_annulus_data)

            phot_table = aperture_photometry(data - bkg_median, aperture, wcs=wcs)
            
            total_source_counts = phot_table['aperture_sum'][0] + aperture.area * bkg_median
            total_source_counts = max(0, total_source_counts)
            
            flux_err_counts = np.sqrt(total_source_counts + (aperture.area * bkg_std**2))
            ...
            if 'PHOTFLAM' in hdul[sci_ext].header: # HST ABmag systems
                ...
                flux_density = (phot_table['aperture_sum'][0] * photflam).to(u.uJy, u.spectral_density(photplam))
                flux_ujy = flux_density.value
                err_density = (flux_err_counts * photflam).to(u.uJy, u.spectral_density(photplam))
                flux_err_ujy = err_density.value
                ...
                flux_density_mjy = phot_table['aperture_sum'][0] * flux_per_pixel_mjy * pixel_area_sr
                flux_ujy = (flux_density_mjy * u.MJy).to(u.uJy).value
                err_density_mjy = flux_err_counts * flux_per_pixel_mjy * pixel_area_sr
                flux_err_ujy = (err_density_mjy * u.MJy).to(u.uJy).value
                ...
                if 'MAGZERO' in hdul[sci_ext].header:
                    ...
                    if 'BUNIT' in hdul[sci_ext].header and 'COUNT' in hdul[sci_ext].header['BUNIT'].upper():
                        counts_per_sec = phot_table['aperture_sum'][0]
                        counts_per_sec_err = flux_err_counts
                        if counts_per_sec > 0:
                            magnitude = -2.5 * np.log10(counts_per_sec) + magzero
                            flux_ujy = 3631e6 * (10**(-0.4 * magnitude))
                            mag_err = (2.5 / np.log(10)) * (counts_per_sec_err / counts_per_sec)
                            flux_err_ujy = flux_ujy * np.log(10) * 0.4 * mag_err
                        else:
                            flux_ujy = 0.0
                            flux_err_ujy = 0.0
            ...
            print(f"  Flux: {flux_ujy:.2f} +/- {flux_err_ujy:.2f} uJy")
    ...
    for index, row in df_in.iterrows():
        ...
        for band_name, band_info in BANDS.items():
            ...
            if image_path and os.path.exists(image_path):
                flux, flux_err = perform_photometry(image_path, coord, instrument)
                galaxy_photometry[f'flux_{band_name}'] = flux
                galaxy_photometry[f'flux_err_{band_name}'] = flux_err
            else:
                galaxy_photometry[f'flux_{band_name}'] = -999.0
                galaxy_photometry[f'flux_err_{band_name}'] = -999.0
